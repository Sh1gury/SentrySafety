...
1. **Hook (~30s).** Open the landing page. One sentence: "Enterprise RAG systems are getting poisoned through their own document supply chains, and they are leaking PII to the LLM on the way in. Sentry Safety is the three-layer firewall that sits in front of the vector store."
...
3. **The save (~90s).** Hit Sanitize. The dashboard shows the three layers in sequence:
   ...
   - Layer 2 lights up: "Denis classifier: prompt_injection (score 0.97). Verdict: warn."
   - Verdict: `block`. **Highlight that the LLM only saw `[PERSON_1] ... [IBAN_1] ...`, never the real values, and that Denis is our own fine-tuned transformer — not a third-party API.**
...
| Denis Space cold start / timeout | Same fallback — `mockLayer2()` shim takes over, `metadata.layer2.demoMode` flips to `true`, UI shows the MOCK badge. |
