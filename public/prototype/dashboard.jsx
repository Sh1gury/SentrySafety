// ============ DASHBOARD PAGE — stats & analytics ============

function DashboardPage({ scans }) {
  const stats = useMemo(() => {
    ...
    const baselinePii = { PERSON: 781, IBAN: 412, EMAIL: 389, PHONE: 334, CREDIT_CARD: 89, IP: 67, LOCATION: 145, ORGANIZATION: 124 };
    ...
    const totalAll = total + 1239;
    const allowAll = allow + 884;
    const blockAll = block + 241;
    const warnAll = warn + 114;

    return {
      totalAll, allowAll, blockAll, warnAll,
      total, allow, block, warn,
  ...
  const threatRows = useMemo(() => {
    return Object.entries(stats.threatCounts).sort((a, b) => b[1] - a[1]).slice(0, 7);
  }, [stats.threatCounts]);

  const piiBreakdown = useMemo(() => {
    return Object.entries(stats.piiCounts).sort((a, b) => b[1] - a[1]);
  }, [stats.piiCounts]);
    ...
    <div className="page">
      ...
      <div className="grid-4" style={{ marginBottom: 20 }}>
        <div className="stat-card neutral">
          ...
          <div className="stat-sub"><span className="delta-up">▲ +{8 + stats.total}</span> today</div>
        </div>
...
function BarRow({ label, value, pct, kind }) {
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, alignItems: 'baseline' }}>
        ...
        <span style={{ display: 'flex', gap: 10, alignItems: 'baseline' }}>
          ...
          <span className="mono" style={{ fontSize: 13, color: `var(--${kind === 'allow' ? 'accent' : kind === 'block' ? 'danger' : 'warn'})`, fontWeight: 600 }}>{value.toLocaleString('en-US').replace(/,/g, ' ')}</span>
        </span>
      </div>
...
function SparkBars() {
  const data = useMemo(() => {
    ...
    for (let i = 0; i < 14; i++) {
      ...
      const allow = total - block - warn;
      out.push({ allow, warn, block });
    }
    return out;
  }, []);
  const maxTotal = Math.max(...data.map(d => d.allow + d.warn + d.block));
  return (
    <div>
