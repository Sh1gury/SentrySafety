// ============ SCAN PAGE — main check screen ============

function ScanPage({ config, setConfig, scans, addScan }) {
  ...
  async function runScan() {
    ...
    if (config.integrity.check_autophagy) {
      await new Promise(r => setTimeout(r, 200));
    }
    setScanStage(4);
    ...
    if (file) {
      if (file.name.endsWith('.zip')) {
        fileThreats.push('zip_bomb');
        verdict = 'block';
      }
      if (/macro|invoice/i.test(file.name) && file.name.endsWith('.docx')) {
        fileThreats.push('macro_present');
        verdict = 'block';
      }
    }
  ...
  return (
    <div className="card">
      ...
      <div className="toggle-row">
        <div className="toggle-info">
          <div className="toggle-label">
            PII masking <span className="layer-tag">L1</span>
          </div>
          <div className="toggle-desc">
            Replaces sensitive data (name, IBAN, phone, email) with tokens before forwarding to the AI
          </div>
        </div>
      ...
      <div className="toggle-row">
        <div className="toggle-info">
          <div className="toggle-label">
            Autophagy check <span className="layer-tag">L3</span>
          </div>
          <div className="toggle-desc">
            Detects AI-generated text to prevent self-replication inside LLM pipelines
          </div>
        </div>
...
// ---------- Entities Panel ----------
function EntitiesPanel({ entities, onToggle, disabled }) {
  return (
    <div className={classNames('card', disabled && 'fade-disabled')} style={disabled ? { opacity: 0.5 } : null}>
      <div className="card-title-row">
      ...
      <div className="tag-row">
        {PII_ENTITIES.map(ent => (
          <Pill
            key={ent}
            on={entities.includes(ent)}
            onClick={() => !disabled && onToggle(ent)}
          >
            {ent}
          </Pill>
        ))}
          ...
          <div className="layer-meta">
            {result.layer3.enabled
              ? <>
                  ...
                  <span className="layer-meta-num">conf {(result.layer3.confidence || 0).toFixed(2)}</span>
                </>
